...
// Define Huge Page constants if not available
#ifndef MAP_HUGETLB
...
static MallocMetadata* mmap_head = NULL; // Head of mmaped blocks list
static void* heap_start = NULL;          // Start of Buddy System Heap
static void* heap_end = NULL;            // End of Buddy System Heap

struct MallocMetadata {
    size_t size;  // Total size of the allocated block (including meta-data).
    bool is_free; // Indicates if the block is used or no
    bool is_mmaped;
    bool is_hugepage; // Indicates if block is a huge page
    MallocMetadata* next;
    MallocMetadata* prev;
};
...
static void* core_malloc(size_t size, bool is_huge_candidate); // Internal allocator
 ...
 * - If size >= 4MB, tries to allocate using Huge Pages (MAP_HUGETLB).
 * - Else, falls back to Part 3 logic (mmap for >128KB, Buddy for <=128KB).
...
void* smalloc(size_t size) {
    ...
    // Proposed Solution: for 'smalloc', if size >= 4MB, use hugepage.
    bool huge_candidate = (size >= 4 * 1024 * 1024);
    return core_malloc(size, huge_candidate);
}

/**
 * @brief Allocates memory for an array and zeroes it.
 * * Challenge 5 Implementation:
 * - Does NOT call smalloc directly to avoid the 4MB threshold rule.
 * - Only uses Huge Pages if the size of ONE block (size) is > 2MB.
...
void* scalloc(size_t num, size_t size) {
    ...
    // Proposed Solution: for 'scalloc', only if block size > 2MB.
    bool huge_candidate = (size > 2 * 1024 * 1024);
    size_t total = size * num;  // Safe now, we've verified no overflow
    void* ptr = core_malloc(total, huge_candidate);
    if (ptr == NULL) {
        return NULL;
    }
...
/**
 * @brief Internal core allocation logic handling Huge Pages, mmap, and Buddy System.
 * * @param size The payload size requested.
 * @param is_huge_candidate Boolean flag triggered by Challenge 5 rules.
 */
static void* core_malloc(size_t size, bool is_huge_candidate) {
    size_t meta_size = sizeof(MallocMetadata);
    size_t total_needed = size + meta_size;

    // 1. Try Huge Page Allocation
    if (is_huge_candidate) {
        // Round up to the nearest multiple of HUGE_PAGE_SIZE
        size_t aligned_size = (total_needed + HUGE_PAGE_SIZE - 1) & ~(HUGE_PAGE_SIZE - 1);
        ...
        if (ptr != MAP_FAILED) {
            ...
            meta->size = aligned_size; // Store the aligned size for munmap
            meta->is_free = false;
            meta->is_mmaped = true;
            meta->is_hugepage = true;
            num_allocated_blocks++;
            num_allocated_bytes += (aligned_size - sizeof(MallocMetadata));

            // Add to mmap list
            meta->next = mmap_head;
            meta->prev = NULL;
            if (mmap_head) mmap_head->prev = meta;
            mmap_head = meta;

            return (meta + 1);
        }
        // If Huge Page fails, fall through to standard logic
    }

    // 2. Standard mmap for Large Allocations (> 128KB)
    if (total_needed > 128 * 1024) {
        void* ptr = mmap(NULL, total_needed, PROT_READ | PROT_WRITE,
        ...
        MallocMetadata* meta = static_cast<MallocMetadata*>(ptr);
        meta->size = total_needed;
        meta->is_free = false;
        meta->is_mmaped = true;
        meta->is_hugepage = false;
        num_allocated_blocks++;
        num_allocated_bytes += (total_needed - sizeof(MallocMetadata));
    ...
    // 3. Buddy System Allocation (<= 128KB)
    if (!is_initialized) {
    ...
    const int ord = get_order(total_needed);
...
void sfree(void* p) {
    ...
    // Check if address is within Buddy System Heap range
    if (p >= heap_start && p < heap_end) {
        // It's a Buddy Block
        auto metadata = static_cast<MallocMetadata*>(p) - 1;
        if (metadata->is_free) return; // Double free protection for Buddy
        ...
        // It must be mmaped (or invalid). Search in mmap_list to verify and remove.
        auto metadata = static_cast<MallocMetadata*>(p) - 1;
        ...
        if (!found) {
            // Pointer not found in mmap list: likely already freed or invalid.
            return;
        }
...
void* srealloc(void* oldp, size_t size) {
    ...
    if (metadata->is_mmaped) {
        if (metadata->size == target_size) {
            return oldp;
        }

        if (metadata->is_hugepage) {
            // Calculate what the aligned size would be for the NEW request
            size_t aligned_target = (target_size + HUGE_PAGE_SIZE - 1) & ~(HUGE_PAGE_SIZE - 1);
            // If the aligned sizes match, we can reuse the block!
            if (aligned_target == metadata->size) {
                return oldp;
            }
        }
...
int initialize_buddy_system() {
    ...
    heap_start = static_cast<void*>(first_block);
    heap_end = reinterpret_cast<void*>(reinterpret_cast<char*>(first_block) + total_size);
